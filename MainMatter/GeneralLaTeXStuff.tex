...
As mentioned above, there is currently not a good implementation of the \href{http://celxj.org/downloads/UnifiedStyleSheet.pdf}{Unified Stylesheet for Linguistics Journals} for \Package{biblatex}.
\href{http://semprag.org/}{\Journal{Semantics \& Pragmatics}} does have an alpha version of an implementation, which is available \href{https://github.com/semprag/biblatex-sp-unified}{here}.

However, I do not like the \href{http://semprag.org/}{\Journal{Semantics \& Pragmatics}} implementation because it requires formatting the \File{.bib} file in a very idiosyncratic manner that prevents it from being portable and usable with other styles.

As part of the \href{https://github.com/lingbib/lingbib}{Lingbib} project that I am working on with Kenneth Hanson (cf.~\S\ref{subsubsubsec:lingbib}), I am developing an implementation based on the \href{http://semprag.org/}{\Journal{Semantics \& Pragmatics}} implementation that does not require the \File{.bib} file to be formatted in an idiosyncratic manner.
It should hopefully be ready for use sometime soon.
I will update this \File{.pdf} when it is ready for use.

In the meantime, you can use the \PackageOption{authoryear-comp} citation style and the \PackageOption{authoryear} bibliography style that are both provided by \Package{biblatex} out of the box.
...
As long as all of the information is there and the style is consistent who cares \ldots{} ?%
}
...
\begin{enumerate}[label={(\roman*)}]
	...
	Nonetheless, if you choose to use \hologo{BibTeX} with \Package{biblatex} for whatever reason, then you will need to run \Engine{biblatex} on the \File{.aux} file instead of running \Engine{biber} on the \File{.bcf} file.%
	}
	}
...
One thing that is nice about doing things this way is that you can use multiple bibliography databases in one document.
That is to say, you can have multiple calls to \mintinline{latex}|\addbibresource| in your preamble, whereas you can only have one call to \mintinline{latex}|\bibliography| in your document with \Package{natbib}.
