...
Instead, you should give glossed abbreviations like this a semantics, since you will presumably want to typeset them all the same way.
To do this, you could declare \mintinline{latex}|\newcommand*[1]{\Gls}{\textsc{#1}}| in your preamble.
Then, you would be able to write \mintinline{latex}|\Gls{nom}| instead of \mintinline{latex}|\textsc{nom}|.
...
\begin{exe}
    \ex[*]{What\Sub{i} did the man that stole t\Sub{i} was caught?}
    \label{ex:bad-English}
\end{exe}
\begin{exe}
    \ex[?]{His\Sub{i} mother loves every\Sub{i} boy no matter what.}
    \label{ex:questionable-English}
\end{exe}
\begin{exe}
    \ex[]{Strong crossover
        \begin{xlist}
            \ex[*]{He\Sub{i} loves everyone\Sub{i}}
            \ex[*]{She\Sub{i} thinks everyone\Sub{i} is smart}
